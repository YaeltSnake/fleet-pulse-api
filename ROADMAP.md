...
All 7 gaps were reviewed with the user via `AskUserQuestion` on 2026-07-13 — every row now has a **Decision** (chosen direction) and a **Status** (implementation progress). See the plan file used for implementation: `Phase 8 Layer 2 — Close Contract Gaps` (approved 2026-07-14/15).

| Order | Gap | Why it matters | Decision | Status |
|---|---|---|---|---|
| 2.1 | `UnitResponse` may be missing a `lastPulseAt` timestamp | Unit Management cards need it per the Phase 7-session UI design | Add `lastPulseAt` to `GET /api/units` (backend enrichment via a new `PulseLogRepository` bulk query, avoids N+1 across the fleet) | ⬜ pending |
| 2.2 | No aggregate "stats today" endpoint (`GET /api/pulse-log` has no summary/count-by-status mode) | Dashboard stat cards (sent/skipped/errors today) have nothing to bind to | New `GET /api/pulse-log/stats` server-side aggregate (single `GROUP BY` query, scoped to pulse-log stats only — "units active" is derived by the frontend client-side from the already-loaded `GET /api/units` response, not blended into this endpoint) | ⬜ pending |
...
| 2.4 | `GET /api/pulse-log?size>100` silently clamps instead of rejecting with `400`, inconsistent with `GET /api/users` | A frontend bug sending `size=10000` fails silently on one endpoint but loudly on the other | `PulseLogController` rejects with `400` via `@Max(100)`/`@Min` + `@Validated`, matching `UserController`. New `HandlerMethodValidationException` handler added to `GlobalExceptionHandler` so both endpoints return our documented `/errors/validation-failed` type URI (previously `UserController`'s 400 used Spring's generic `about:blank`, undocumented) | ⬜ pending |
...
| 2.7 | `PUT /api/users/{id}` requires a new `rawPassword` on every call, even for a role/active-only change | Forces the ADMIN to invent/retype a password just to flip a dropdown | New `PATCH /api/users/{id}` endpoint (role + active only, no password field in the DTO at all — not just ignored, structurally absent). `PUT` unchanged for full-replace edits. New `UserManagementUseCase.updateRoleAndActive()`. | ⬜ pending |
